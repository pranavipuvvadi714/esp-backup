module divider_unit (
    ...
    always @(posedge clock) begin  //SEQ logic
        if (rst == 1'b0) begin
            ...
            divided_out_save    <= freeze ? divided_out_wsave : divided_out;//_wsave;//Fix MC
    ...
    always @* begin : COMBO  //Combinational logic
        ...
        if (flag_end == 1 && zerozero_flag == 0) begin
            ...
            packet_out_val[6:0] = sign_save ? -$signed({1'b0,divided_out_wsave}) : $signed({1'b0,divided_out_wsave});
            token_delta = (sign_save) ? $signed({1'b0,divided_out_wsave}) : -$signed({1'b0,divided_out_wsave});
